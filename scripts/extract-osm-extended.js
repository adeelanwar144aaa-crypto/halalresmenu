/**
 * UK restaurant extractor (extended OSM strategies) → Supabase.
 * Cuisine tags + name keywords (beyond diet:halal=yes).
 * Run: npm run extract-osm2
 */
...
(
  node["amenity"="restaurant"]["cuisine"~"pakistani|indian|bangladeshi|turkish|lebanese|arabic|persian|afghan|moroccan|halal"](around:${RADIUS_M},${lat},${lon});
  node["amenity"="fast_food"]["cuisine"~"pakistani|indian|bangladeshi|turkish|kebab|chicken"](around:${RADIUS_M},${lat},${lon});
  node["amenity"="restaurant"]["name"~"halal|kebab|karahi|biryani|tandoori|masala|curry|spice|balti|tikka",i](around:${RADIUS_M},${lat},${lon});
);
out body;`;
}
...
async function fetchOverpassForCity(lat, lon) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      return await postOverpass(query);
    } catch (err) {
      lastError = err;
      if (attempt < MAX_RETRIES) {
        const waitMs = RETRY_DELAY_MS + retryJitterMs();
        console.warn(
          `  Overpass attempt ${attempt} failed: ${err.message}. Retrying in ${(waitMs / 1000).toFixed(1)}s…`
        );
        await sleep(waitMs);
      }
    }
...
async function main() {
  ...
  console.log(
    `Starting extended OSM UK extract for ${totalCities} cities…\n`
  );
  ...
  for (let i = 0; i < UK_CITIES.length; i++) {
    ...
    try {
      ...
      console.error(`Skipping ${city.name}: Overpass failed — ${message}`);
  ...
  writeFileSync(
    resolve(process.cwd(), "summary-osm-extended.json"),
    JSON.stringify(
      {
  ...
  if (failedCities.length > 0) {
    writeFileSync(
      resolve(process.cwd(), "failed-cities-osm-extended.json"),
